...
<%
    ...
    Skill skill = ConnectDb.getSkill(skillId);
%>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <nav class="navbar navbar-dark bg-primary displayright" style="width: 100%;" >
        ...
        <a class="navbar-brand nav-link hoverEffect" href="categories.jsp"><i class="fas fa-th-large"></i> Catégories</a>
     ...
     <div class="form-creer-competence">
        <h1>Edit une competence</h1>
        <form action="EditServlet" method="post">
            <input type="text" id="id" name="id" value="<%= skill.getId() %>" required><br>
            <label for="name">Nom de la competence:</label>
            <input type="text" id="name" name="name" value="<%= skill.getName() %>" required><br>
        
            <label for="description">Description:</label>
            <textarea id="description" name="description"  required><%= skill.getDescription() %></textarea><br>
        
            <label for="level">Niveau requis:</label>
            <select id="level" name="level">
                <option value="Debutant" <%= skill.getLevel().equals("Debutant") ? "selected" : "" %>>Debutant</option>
                <option value="Intermadiaire" <%= skill.getLevel().equals("Intermadiaire") ? "selected" : "" %>>Intermediaire</option>
                <option value="Avance" <%= skill.getLevel().equals("Avance") ? "selected" : "" %>>Avance</option>
            </select><br>
        
            <label for="domain">Domaine:</label>
            <select id="domain" name="domain">
                <option value="Sciences de donnees" <%= skill.getDomain().equals("Sciences de donnees") ? "selected" : "" %>>Sciences de donnees</option>
                <option value="Securite" <%= skill.getDomain().equals("Securite") ? "selected" : "" %>>Securite</option>
                <option value="Statistiques" <%= skill.getDomain().equals("Statistiques") ? "selected" : "" %>>Statistiques</option>
            </select><br>
